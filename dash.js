...
app.get("/profile/:id", async(req, res) => {
  ...
  let vip = client.db.get(`vip.${req.params["id"]}`) ? `<i class="far fa-check-circle" style="color:GREEN"></i>`: `<i class="far fa-times-circle" style="color:RED"></i>`
...
app.get("/yonetim/:id/ayar", async(req, res) => {
  ...
  app.post("/admin/gold-uye", async(req, res) => {
    if(!client.ayarlar.gelistirici.includes(req.user.id) ) return res.redirect('/')
 if(req.body["gold-id"]) {
      client.db.set(`vip.${req.body["gold-id"]}`, true)
      client.db.set(`viptime.${req.body["gold-id"]}`, Date.now() + ms(req.body["gold-time"] + "d"))
    }else if(req.body["usr-id"]) {
      client.db.delete(`vip.${req.body["gold-id"]}`)
      client.db.delete(`viptime.${req.body["gold-id"]}`)
    }
